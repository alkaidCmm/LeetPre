package com.cmm.jzOffer._66hasPath;
...
public class Solution {
	...
	public static boolean hasPath(char[] matrix, int rows, int cols,
		...
		for (int i = 0; i < rows; i++)
			for (int j = 0; j < cols; j++)
				if (hasPathCore(matrix, rows, cols, str, visited, i, j,
						pathLength))
					return true;
		return false;
	}
	...
	private static boolean hasPathCore(char[] matrix, int rows, int cols,
			char[] str, boolean[] visited, int row, int col, int[] pathLength) {
		...
		if (row >= 0 && row < rows && col >= 0 && col < cols
			...
			// 按左上右下进行回溯
			hasPath = hasPathCore(matrix, rows, cols, str, visited, row,
					col - 1, pathLength)
					|| hasPathCore(matrix, rows, cols, str, visited, row - 1,
							col, pathLength)
					|| hasPathCore(matrix, rows, cols, str, visited, row,
							col + 1, pathLength)
					|| hasPathCore(matrix, rows, cols, str, visited, row + 1,
							col, pathLength);
		...
		System.out
				.println(hasPath(
						"ABCEHJIGSFCSLOPQADEEMNOEADIDEJFMVCEIFGGS"
								.toCharArray(),
						5, 8, "SLHECCEIDEJFGGFIE".toCharArray()) + "[true]");// true
